...
export default function App() {
  ...
  // Define your Cloudflare Worker URL here
  const CLOUDFLARE_WORKER_URL = "https://virtual-try-on.fassikafikre.workers.dev/"; // <<-- YOUR WORKER URL
  ...
  const analyzeImage = async (file) => {
    ...
    try {
      ...
      const payload = {
        model: "gemini-2.5-flash-preview-05-20", // Model specified for the Worker
        contents: [{
          parts: [
      ...
      const response = await fetch(CLOUDFLARE_WORKER_URL, { // Using Worker URL
        method: 'POST',
  ...
  const generateImage = async () => {
    ...
    if (mode === 'accessory') {
      ...
      payload = {
        model: "gemini-2.5-flash-image-preview", // Model specified for the Worker
        contents: [{
          parts: [
            { text: initialPrompt },
            {
    ...
    let response;
    try {
      response = await fetch(CLOUDFLARE_WORKER_URL, { // Using Worker URL
        method: 'POST',
        headers: {
  ...
  const updateLook = async () => {
    ...
    try {
      ...
      const apiResponse = await fetch(CLOUDFLARE_WORKER_URL, { // Using Worker URL
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
